...
open class GenerateModelTask : DefaultTask() {
    ...
    @get:Internal
    var generatedFileNames: MutableList<String> = mutableListOf<String>()
    ...
    fun doWork() {
        ...
        try {
            ...
            try {
                ...
                val generatedFiles: List<File> = DefaultGenerator().opts(clientOptInput).generate()

                generatedFiles.forEach {
                    if (it.absolutePath.endsWith(".kt")) {
                        generatedFileNames.add(it.absolutePath)
                    }
                }
